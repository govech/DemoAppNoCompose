...
class ExceptionHandler @Inject constructor(
    ...
    private fun convertToAppException(exception: Throwable): AppException {
        return when (exception) {
            ...
            // 业务异常转换
            is IllegalArgumentException -> AppException.BusinessException.InvalidParameterError(
                exception.message ?: "参数错误"
            )
            is IllegalStateException -> AppException.BusinessException.BusinessRuleViolationError(
                exception.message ?: "业务规则违反"
            )
            
            // 系统异常转换
            is OutOfMemoryError -> AppException.SystemException.OutOfMemoryError(exception)
            is SecurityException -> AppException.SystemException.PermissionError(
                exception.message ?: "系统权限被拒绝"
            )
            
            // 其他异常
            else -> AppException.SystemException.UnknownError(
    ...
    private suspend fun tryRecover(
        ...
        // 找到合适的恢复策略
        val strategy = recoveryStrategies.find { it.canHandle(exception) }
