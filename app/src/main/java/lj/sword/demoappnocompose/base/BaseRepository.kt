...
abstract class BaseRepository {
     ...
     * @return Flow<T> 数据流
     */
    protected fun <T> executeRequest(block: suspend () -> BaseResponse<T>): Flow<T> =
        flow {
            try {
                val response = block()
                    ...
                    val exception =
                        when (response.code) {
                            ...
                            in 500..599 ->
                                ...
                                AppException.NetworkException.HttpError(
                                    response.code,
     ...
     * @return T 数据
     */
    protected suspend fun <T> executeRequestDirect(block: suspend () -> BaseResponse<T>): T {
        return try {
            val response = block()
     ...
     * @return Flow<T> 数据流
     */
    protected fun <T> executeLocal(block: suspend () -> T): Flow<T> =
        flow {
            try {
                emit(block())
            } catch (e: Exception) {
